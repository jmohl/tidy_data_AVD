...
function [p] = g_read_preamble4(fid,VSG_FLAG,USR_PREAMB_ELEMENTS);
...
if p.DATATYPE_VERSION > 1
   ...
   p.LOWPASSFILTER1      = temp(19);
   p.LOWPASSFILTER2      = temp(20);
   p.LOWPASSFILTER3      = temp(21);
   p.LOWPASSFILTER4      = temp(22);
   ...
   p.PROBLEM             = temp(26);
	...
	if VSG_FLAG % Additional bitmap file names appended 11/15/04
        p.BITMAP17 = tostr(fread(fid, 10, CHAR));
        p.BITMAP18 = tostr(fread(fid, 10, CHAR));
        p.BITMAP19 = tostr(fread(fid, 10, CHAR));
  	 	p.BITMAP20 = tostr(fread(fid, 10, CHAR));
        p.BITMAP21 = tostr(fread(fid, 10, CHAR));
  	 	p.BITMAP22 = tostr(fread(fid, 10, CHAR));
        p.BITMAP23 = tostr(fread(fid, 10, CHAR));
        p.BITMAP24 = tostr(fread(fid, 10, CHAR));
        p.BITMAP25 = tostr(fread(fid, 10, CHAR));
        p.BITMAP26 = tostr(fread(fid, 10, CHAR));
	end
