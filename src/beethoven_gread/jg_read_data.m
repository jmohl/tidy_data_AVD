...
%		eddie 12.12.05 eyegain changed to gainx and gainy with backward compatibility
%		eddie 04.11.06 added preamble4 which handles preamble version 7, incorporates additional variables 
%		eddie 03.31.07 added preamble5 which handles preamble version 8, incorporates groh lab additional variables for sound features
function [dat,fid,num_trials] = g_read_data(fname,start_trial,end_trial,rank);
...
for i = index
   ...
   case {7}
      temp = g_read_preamble4(fid,VSG_FLAG,USR_PREAMB_ELEMENTS);
   case (8)
      temp = jg_read_preamble5(fid,VSG_FLAG,USR_PREAMB_ELEMENTS);
   otherwise
   ...
   %eddie changed GAIN to gainx and gainy adding an additional param
   % Get the eye data
   
   if (preamble_version > 2)
