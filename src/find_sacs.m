...
%end: velocity < 25 deg/s, at least 30 ms before next saccade detected
%magnitude: at least 1 degree
%adjustment: 30 ms is added to end time in order to get past slowdown time
...
min_duration = 15;%multiply by 2 to get time in ms
start_vel = 50; %degrees per second for start of saccades
end_vel = 25; %degrees per second for end of saccades
min_magnitude = 1; %minumum magnitide in degrees to count saccade
...
high_times = find(eye_vel>start_vel); %eye velocity > 50 deg/s
shifted = vertcat(high_times(2:end),0);
transition_times = high_times(find(abs(shifted-high_times) > min_duration)); %detect time points where the saccade slows down for at least 30 ms
sac_end_times=[];
sac_start_times=[];
for t = 1:length(transition_times);
    transition = transition_times(t);
    slow_times = find(eye_vel<end_vel); %find times where saccade is below 25 deg/s
    ...
    if ~isempty(slow_times(find(slow_times > transition,1)))    %first slow time after transition
        ...
        sac_end = slow_times(find(slow_times > transition,1))+15;%+15 makes endpoint more accurate (instead of being on the curve, is on the flat)
        if sac_end < length(HEyeTrace) %dont use endpoints that go past the end of the trial
            ...
            if magnitude > min_magnitude & max(abs(VEyeTrace(sac_start:sac_end))) < 35  %saccade must have a magnitude of at least 2 degrees, otherwise skipped, also skipping blinks (result in very high V amplitude when using eye tracker)
